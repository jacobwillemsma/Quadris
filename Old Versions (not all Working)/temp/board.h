...
class Board {
	...
	int currentRow, currentCol; // current row and column of the bottom left of block
	int * currentPosition; // hold the current Block position (not changed until a Block move is cleared)
	int * nextPosition; // holds the next move, or potential move for the current Block
	...
	void gameStart(int lvl); // starts a game
	...
	Block getBlock(std::string b); // returns a block
  ...
  public:
	...
	friend std::ostream &operator<<(std::ostream &out, Board b); // prints the Grid
};
